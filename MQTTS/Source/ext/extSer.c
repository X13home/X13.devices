...
ISR(SER_UDRE_VECT)
{
  if(serTxPos < serTxLen)                         // Check if all data is transmitted
    SER_DATA = serTxBuf[serTxPos++];
  else
  {
    serTxLen = 0;
    SER_DISABLE_DREINT();                         // Disable UDRE interrupt
  }
}
#endif  //  EXTSER_TX_USED
...
uint8_t serRegisterOD(indextable_t *pIdx)
{
  ...
  if(pIdx->sidx.Type == objSerTx)
  {
    ...
    if(SER_IS_ENABLED() == 0)    // Serial not configured
    {
      SER_ENABLE();
